...
Each change has a 'up' and 'down' .sql file. Files in `APPPATH/migrations/ups/` progress the database forward in the development timeline, files in `APPPATH/migrations/downs` bring it backwards.

## Example

### APPPATH/migrations/ups/2.sql
...
### APPPATH/migrations/downs/2.sql
...
2. You create the file `APPPATH/migrations/ups/42.sql` and write the necessary SQL (`CREATE TABLE ...`).
3. Next, You add the reverse SQL to the file `APPPATH/migrations/downs/42.sql (`DROP TABLE ...`)
